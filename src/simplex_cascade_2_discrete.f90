...
call amoeba(xx, yy, 3, 2, 2, 1.E-8_dp, acov_SSE, iter)
...
! of guessed parameters. Will be the workhorse when doing the search.
	...
	do while (minval(event_count) < event_min)
		...
		call update_propensity(propensity, x, lmbda, ell, beta)
		...
		if (t .ge. (tlast + tdisc)) then
			...
			do i = 1, floor((t-tlast)/tdisc)
				...
				guess_Rwindow(:, nacov) = guess_R(x, lmbda(2), guess_params(1), guess_params(2), beta(2))
...
pure function guess_R(x, lmbda, ell1, ell2, beta) result(rate)
	integer, intent(in) :: x(2)
	real(dp), intent(in) :: lmbda, beta, ell1, ell2
	real(dp) :: rate(2)
	real(dp), parameter :: tiny = 1.E-8_dp
	
	! Rate in
	rate(1) = 1._dp * lmbda * (x(1)+tiny) ** ell1
	! Rate out
	rate(2) = 1._dp * beta * (x(2)+tiny) ** ell2
end function


pure function production_rates(x, lmbda, ell) result(rate)
	integer, intent(in) :: x(2)
	real(dp), intent(in), dimension(2) :: lmbda, ell
	real(dp) :: rate(2)
...
subroutine update_propensity(prop, x, lmbda, ell, beta)
	integer, intent(in), dimension(2) :: x
	real(dp), intent(in), dimension(2) :: lmbda, ell, beta
	real(dp), dimension(4), intent(inout) :: prop
	real(dp), dimension(2) :: Rin, Rout
	
	Rin = production_rates(x, lmbda, ell)
