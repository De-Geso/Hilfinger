...
! Output filename
character(len=*), parameter :: fname = "noise_enhancing_"
! Epsilon
...
integer, parameter, dimension(3) :: abund_max = [2**7, 2**7, 2**7]
! Number of abundance updates to remember for autocovariance. Reducing this gives big time savings.
integer, parameter :: nwindow = 2000
! Number of points in autocovariance.
integer, parameter :: nacov = 2**7
! Maximum time lag for autocovariance.
real(dp), parameter :: maxlag = 4._dp
! Time step for autocovariance.
real(dp), parameter :: acov_tstep = 1._dp*maxlag/(nacov-1)

! System parameters ====================================================
! Production rates
real(dp), parameter, dimension(3) :: lmbda = [50._dp, 1._dp, 80._dp]
! Decay rates
real(dp), parameter, dimension(3) :: beta = [1._dp, 1._dp, 1._dp]
! Hill function parameters
real(dp), parameter, dimension(3) :: k = [37._dp, 37._dp, 40._dp]
real(dp), parameter, dimension(3) :: n = [6._dp, 6._dp, 2._dp]
real(dp), parameter, dimension(3) :: c = [15._dp, 1._dp, 0._dp]
...
! Windows
real(dp) :: twindow(nwindow) = 0._dp, guess_Rwindow(2, nwindow) = 0._dp
integer(dp) :: xwindow(nwindow) = 0
! Covariance arrays
real(dp) :: acov(3, nacov), acov_mean(3, 2, nacov)=0._dp, acov_mean2(3, nacov)=0._dp
! Timers
real(dp) :: tacov
...
integer :: x(3)=0, maxX(3) = 0, event_count(6) = 0, event
! Counters
integer :: i
...
do while (minval(event_count) < event_min)
	...
	guess_Rwindow(:, :nwindow-1) = guess_Rwindow(:, 2:nwindow)
	guess_Rwindow(:, nwindow) = [guess_R(x, lmbda(3), k(3), n(3), beta(3))]
	...
	if (twindow(1) > eps) then
		...
		!$omp sections
			...
			call update_autocovariance(acov_mean2(1,:), acov_mean(1,1,:), acov_mean(1,2,:), &
				real(xwindow, dp), real(xwindow, dp), twindow, maxlag, nwindow, nacov, acov_tstep, tstep)
			!$omp section
			! Autocovariances of Rin and component
			call update_autocovariance(acov_mean2(2,:), acov_mean(2,1,:), acov_mean(2,2,:), &
				guess_Rwindow(1,:), real(xwindow, dp), twindow, maxlag, nwindow, nacov, acov_tstep, tstep)
			!$omp section
			! Autocovariances of Rout and component
			call update_autocovariance(acov_mean2(3,:), acov_mean(3,1,:), acov_mean(3,2,:), &
...
do i = 1, 3
	call finalize_autocovariance(acov_mean2(i,:), acov_mean(i,1,:), acov_mean(i,2,:), tacov, acov(i,:))
end do
...
! Output data to file
	...
	character(len=*), parameter :: directory = "data/", prefix="3D_", suffix = ".dat"


	open(newunit=io, file=fname, status='new', action='write')
	! Write metadata
	write(io,*) "# Program Metadata"
	write(io,*) "# Program: single_path_3D.f90"
!	write(io,*) "# Description: &
!		3 component system simulation unnormalized correlations and derivative of App from correlation relations."
	write(io,*) "# Creation date: ", fdate()
	write(io,*) "# Seed: ", rseed
	write(io,*) "# Min events: ", event_min
	write(io,*) "# Maximum abundance: ", abund_max
	write(io,*) "# Autocovariance window size: ", nwindow
	write(io,*) "# Autocovariance vector length", nacov
