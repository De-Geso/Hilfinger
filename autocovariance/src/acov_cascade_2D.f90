...
integer, parameter :: nwindow = 300
! Time step for autocovariance.
real(dp), parameter :: acov_tstep = 0.1
! Number of points in autocovariance.
integer, parameter :: nacov = ceiling(umax/acov_tstep)+1
...
! Windows
real(dp) :: guess_Rwindow(2,nwindow)=0._dp, twindow(nwindow)=0._dp
...
! Variables ============================================================
! Timers
real(dp) :: t=0._dp, tstep, tacov=0._dp
! Probability distribution
real(dp), dimension(abund_max(1), abund_max(2)) :: pcond = 0._dp
! Moments
type(OnlineCovariance) :: stats(2,2), cov22
...
do while (minval(event_count) < event_min)
	...
	! Online calculation of means and covariance balance checks
	do i = 1, 2
	do j = 1, 2
		call stats(i,j)%update(real(x(i),dp), propensity(2*j) - propensity(2*j-1), tstep)
	end do
	end do
	call cov22%update(real(x(2),dp), real(x(2),dp), tstep)

	! Keep window for autocovariance calculations
	! Time
	...
	guess_Rwindow(:, nwindow) = guess_R(x, lmbda(2), guess_params(1), guess_params(2), c(2), beta(2))
	...
	if (twindow(1) > eps) then
		...
		!$omp sections
			...
			call update_autocovariance(acov_mean2(1,:), acov_mean(1,1,:), acov_mean(1,2,:), &
				real(xwindow, dp), real(xwindow, dp), twindow, umax, nwindow, nacov, acov_tstep, tstep)
			!$omp section
			! Autocovariances of Rin and component
			call update_autocovariance(acov_mean2(2,:), acov_mean(2,1,:), acov_mean(2,2,:), &
				guess_Rwindow(1,:), real(xwindow, dp), twindow, umax, nwindow, nacov, acov_tstep, tstep)
			!$omp section
			! Autocovariances of Rout and component
			call update_autocovariance(acov_mean2(3,:), acov_mean(3,1,:), acov_mean(3,2,:), &
				guess_Rwindow(2,:), real(xwindow, dp), twindow, umax, nwindow, nacov, acov_tstep, tstep)
		!$omp end sections
		!$omp end parallel
	end if
