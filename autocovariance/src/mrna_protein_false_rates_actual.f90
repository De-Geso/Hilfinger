...
!	Rwindow(:, nwindow) = [Pbirth(real(mp,dp)), Pdecay(real(mp,dp))]
	...
	fake_Rwindow(:, nwindow) = [Pbirth(real(mp,dp), ell), Pdecay(real(mp,dp), ell)]
...
! LAMBDA AND ALPHA LIVE IN R AND Pbirth RESPECTIVELY.
	...
	prop(3) = 1._dp * Pbirth(real(x,dp), ell)	! Make protein
	prop(4) = 1._dp * Pdecay(real(x,dp), ell)	! Degrade protein
end function
...
pure function Pbirth(x, var) result(f)
! Production function for protein.
	real(dp), intent(in) :: x(2), var(2)
	real(dp) :: f
	associate(m => x(1), p => x(2))
	!	f = 1._dp * hill_pos(m, k, n)
		f = 1._dp * x(1)**var(1)
...
pure function Pdecay(x, var) result(f)
...
! Output results.
	...
	write(prefix, '(A, F5.3, A, F5.3, A)') "infer_power_law_l1_", param(1), "_l2_", param(2), "_"
